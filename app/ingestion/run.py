...
def _get_embedding_fn():
    ...
    try:
        ...
        client = APIClient(credentials=credentials, project_id=os.environ["WATSONX_PROJECT_ID"])
        embeddings = Embeddings(
            model_id=os.environ["WATSONX_EMBEDDING_MODEL_ID"],
            api_client=client,
        )

        def real_embedding_fn(text: str) -> list[float]:
...
def run(manifest_path: Path, dry_run: bool = False) -> None:
    """
    Main ingestion loop.
    ...
    logger.info("Manifest: %s  |  Dry-run: %s", manifest_path, dry_run)
    logger.info("=" * 60)
...
def main() -> None:
    ...
    run(manifest_path=args.manifest, dry_run=args.dry_run)
